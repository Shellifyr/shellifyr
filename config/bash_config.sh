...
function _fmt_debug {
  printf '%sDEBUG: %s%s' "${FMT_BOLD}" "${FMT_BLUE}" "$*" 
  printf '%s\n' "$FMT_RESET"
}
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  if [[ $DEBUG_MODE == true ]]; then
    _fmt_debug "DEBUG: sourced '$CONFIG_FILE'."
  fi
else
...
for plugin in "${PLUGINS[@]}"; do 
  ...
  if [[ -f "$PLUGIN_FILE" ]]; then
    ...
    if [[ -z "$COMPATIBILITY" || "$COMPATIBILITY" == *"$CURRENT_SHELL"* ]]; then 
      if [[ $DEBUG_MODE == true ]]; then
        _fmt_debug "DEBUG: loading '$plugin' plugin..."
      fi
      source "$PLUGIN_FILE"
      if [[ $DEBUG_MODE == true ]]; then
        _fmt_debug "DEBUG: plugin '$plugin' successefully loaded."
      fi
    else 
...
if [[ -f "$THEME_FILE" ]]; then
  ...
  if [[ -z "$COMPATIBILITY" || "$COMPATIBILITY" == *"$CURRENT_SHELL"* ]]; then 
    if [[ $DEBUG_MODE == true ]]; then 
      _fmt_debug "DEBUG: '$SHYR_THEME' loading."
    fi
    source "$THEME_FILE"
    ...
    if [[ $DEBUG_MODE == true ]]; then 
      _fmt_debug "DEBUG: '$SHYR_THEME' theme successfully loaded."
    fi
  else 
